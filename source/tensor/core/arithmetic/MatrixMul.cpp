                ...
                XTensor * c, DTYPE alpha, DTYPE beta, XPRunner * parallelRunner)
{
	...
	//std::cout << "an am bn bm cn cm" << " " << an << " " << am << " " << bn << " " << bm << " " << cn << " " << cm << //std::endl;
	...
	//std::cout << "abs bbs cbs arb brb crb ab bb cb" << " " <<
	//	aBlockSize << " " << bBlockSize << " " << cBlockSize << " " << aRealBlockSize
	//	<< " " << bRealBlockSize << " " << cRealBlockSize << " " << aBlockNum << " "
	//	<< bBlockNum << " " << cBlockNum << //std::endl;
	...
	//std::cout << "adimsize" << " " << aDimSize[0] << " " << aDimSize[1] << //std::endl;
...
void MatrixMul(const XTensor &a, MATRIX_TRANS_TYPE transposedA,
               ...
               DTYPE alpha, DTYPE beta, XPRunner * parallelRunner) // alpha=1 beta=0 runner=null
{
    CheckNTErrors(a.dataType == b.dataType, "Input tensors should have the same data type!");
    CheckNTErrors(a.order >= 2 && b.order >= 2, "Input tensors must have a order >= 2!");
