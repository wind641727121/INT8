        ...
        if (!a->isSparse && !b->isSparse) {
                ...
                c->dataType == DEFAULT_DTYPE)
            {
                ...
                if( c == a){
					...
					///对int的支持
				if (a->dataType == X_INT && a->dataType == b->dataType&&a->dataType == c->dataType)
				{
					register int i = 0, num = a->unitNum; //直接四个一起做 最后不足四个在另算
					int* cp = (int*)c->data;
					int* ap = (int*)a->data;
					int* bp = (int*)b->data;
					int x = num - 4, tmp = beta;
					for (i; i < x; i += 4)
					{
						cp[i] = ap[i] + bp[i] * tmp;;
						cp[i + 1] = ap[i + 1] + bp[i + 1] * tmp;;
						cp[i + 2] = ap[i + 2] + bp[i + 2] * tmp;;
						cp[i + 3] = ap[i + 3] + bp[i + 3] * tmp;;
					}
					while (i < num){	
						cp[i] = ap[i] + bp[i] * tmp;;
						++i;
