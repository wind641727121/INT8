...
float tmp;
float *da, *db, *dc;
void My_Mul(XTensor &a, XTensor &b, XTensor &c)
{
	da = (float*)a.data, db = (float*)b.data, dc = (float*)c.data;
	for (register int k = 0; k < a.dimSize[1]; ++k)
	{
		for (register int i = 0; i < a.dimSize[0]; ++i)
		{
			tmp = *(da + i * (a.dimSize[1]) + k);
			//if (tmp == 0) continue;
			for (register int j = 0; j < b.dimSize[1]; ++j)
			{
				*(dc + i * c.dimSize[1] + j) += *(db + k * b.dimSize[1] + j)*tmp;
			}
		}
	}
}
...
void LoadArgs(int argc, const char ** argv, FNNModel &model)
{
    ...
    for(int i = 0; i < argc; i++){
		...
		//	std::cout << "tset " << model.n << std::endl;
            fprintf(stderr, " -n=%d\n", model.n);
        }
...
void Train(const char * train, bool isShuffled, FNNModel &model)
{
    ...
    for(epoch = 0; epoch < nEpoch; epoch++){
        ...
        while(ngramNum > 0){
            ...
            ngramNum = LoadNGrams(file, model.n, ngrams, sentBatch, wordBatch); //每次取128个
			//printf("ngaram = %d \n", ngramNum);
            if (ngramNum <= 0)
                break;
			...
			//把n_gram的前n-1维转成one-hot形式  每个input是128*V的大小
			//n_garm 是128*3形式 
...
void Forward(XTensor inputs[], XTensor &output, FNNModel &model, FNNNet &net)
{
    ...
    for(int i = 0; i < n - 1; i++){ //换成词向量
		...
		XTensor xi, si, xw, sw,xe,se;
		
		InitTensor2D(&xe,input.dimSize[0],w.dimSize[1],X_INT8);
		InitTensor2D(&se, input.dimSize[0],1);
		 
	//	double flag = 0;
		//for (int i = 0; i < w.dimSize[1]; i++)
	//		flag += * ((float*)w.data + i);
	//	std::cout << flag;
	//	getchar();
		
	//	Initinput(input, xi, si,min(input.dimSize[0], 8)); // xi 8*16400 input 8*16400 
 
	//	InitMap_Row(w,xw,sw,12); //xw 16400*100 sw16400*1
	 

		MatrixMul(input, X_NOTRANS, w, X_NOTRANS, embedding);
	 
	//	Mymul_int8(xi, xw, xe, si, sw, se, min(input.dimSize[0], 8));
	 
	//	UnMapping(embedding, xe, se, min(embedding.dimSize[0], 8));
    ...
    InitModelTensor2D(net.embeddingCat, batchSize, (n - 1) * model.eSize, model); //8*400
    Concatenate(eList, net.embeddingCat, 1);
	XTensor xembeding, sembeding;  
	XTensor xneth[MAX_HIDDEN_NUM + 5], sneth[MAX_HIDDEN_NUM + 5];
//	InitMap_Row(net.embeddingCat, xembeding, sembeding, min(net.embeddingCat.dimSize[0], 8));
    ...
    for(int i = 0; i < depth; i++){
		...
		XTensor &xh_pre = i == 0 ? xembeding : xneth[i - 1];
		XTensor &sh_pre = i == 0 ? sembeding : sneth[i - 1];
        XTensor &w = model.hiddenW[i],sw,xw;
        XTensor &b = model.hiddenB[i],sb,xb;
		 
        XTensor &h = net.hiddens[i]; XTensor &sh = sneth[i]; XTensor &xh = xneth[i];
        XTensor &s = net.hiddenStates[i],ss,xs;

        InitModelTensor2D(h, batchSize, model.hSize, model);
        InitModelTensor2D(s, batchSize, model.hSize, model);
	 
		 
		// InitMap_1(w, xw, sw,8); //400*256 1*1
		//std::cout << "hw ";
		//for (int i = 0; i < 10; i++) std::cout << *((float*)w.data + i) << std::endl;
		 InitTensor2D(&xs, s.dimSize[0], s.dimSize[1], X_INT8);
		 InitTensor2D(&ss, sh_pre.dimSize[0], sw.dimSize[1]);
        /* generate hidden states of layer i: 
           s = h_pre * w    */

		 
		// Mymul_int8(xh_pre,xw, xs,sh_pre,sw, ss, min(xh_pre.dimSize[0],8)); //映射乘法
		...
		//s = Myadd_int8(xs, b2D, ss, min(xs.dimSize[0], 8));
	...
	//	 InitMap_Row(h, xh, sh, min(h.dimSize[0], 8));
       ...
       TODO: we add bias term here */
    {
		...
		XTensor &xh_last = depth > 0 ? xneth[depth - 1] : xembeding; //8*256
		XTensor &sh_last = depth > 0 ? sneth[depth - 1] : sembeding; //8*1
        XTensor &w = model.outputW,xw,sw;  //256*16400
        XTensor &b = model.outputB,sb,xb;
        XTensor  ss,xs;
        XTensor &y = output;

      //  InitModelTensor2D(s, batchSize, model.vSize, model); //8*16400
        InitModelTensor2D(y, batchSize, model.vSize, model);
	 
		//  InitMap_Row(w, xw, sw, min(w.dimSize[0], 8));  // sw 1*16400
		// InitMap_1(w, xw, sw, min(w.dimSize[0], 8));
	//	 InitMap_Col(w, xw, sw, min(w.dimSize[0], 8));
 
		 
		 
		InitTensor2D(&xs, y.dimSize[0], y.dimSize[1], X_INT8);
		InitTensor2D(&ss, sh_last.dimSize[0], sw.dimSize[1]);
        /* s = h_last * w  */
	//	std::cout << "yw ";
	//	for (int i = 0; i < 10; i++) std::cout << *((float*)w.data + i) << std::endl;
		// Mymul_int8(xh_last, xw, xs,   sh_last , sw, ss, 8);
		 
        MatrixMul(h_last, X_NOTRANS, w, X_NOTRANS, y);
		 
        XTensor b2D;
        InitTensor(&b2D, &y);
        Unsqueeze(b, b2D, 0, batchSize);
	//	y=Myadd_int8(xs, b2D, ss, min(xs.dimSize[0], 8));
		  Sum(y, b2D, y);
...
void Test(const char * test, const char * result, FNNModel &model)
{
    ...
    while (ngramNum > 0) {
		...
		{
			double elapsed = GetClockSec() - startT;
			XPRINT1(0, stderr, "[INFO] ppl=%.2f ", exp(loss / wordCount));
			XPRINT3(0, stderr, "  (took %.1fs, sentence=%d and ngram=%d)\n",
